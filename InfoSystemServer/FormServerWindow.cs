...
namespace 软件系统服务端模版
{
    public partial class FormServerWindow : Form
    {
        ...
        private void Form1_Load( object sender, EventArgs e )
        {
            ...
            toolStripStatusLabel1.Text = $"The software copyright belongs to {SoftResources.StringResouce.SoftCopyRight}";
        ...
        private void 启动服务器ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (!IsSystemStart)
            {
                ...
                启动服务器ToolStripMenuItem.Text = "Started"; 
        ...
        private void MaintenanceInitialization()
        {
            ...
            {
                label3.Text = "Login enabled";                  // Login Enabled True
                label3.BackColor = Color.LimeGreen;
            }
            else
            {
                label3.Text = "Under maintenance";                  // Login Enabled False
        ...
        /// <summary>
        /// Receives byte data from the client
        /// </summary>
        /// <param name="session">Network state</param>
        /// <param name="customer">Byte data, use based on actual needs</param>
        /// <param name="data">Byte data from the client</param>
        ...
        /// <summary>
        /// Received data from the client, maintenance validation, update validation, etc., should be handled here
        /// </summary>
        /// <param name="session">Client's address</param>
        /// <param name="handle">Custom instruction header, optional, data can be used to differentiate instead</param>
        /// <param name="data">Received data from the server</param>
        ...
        #region 1.1.X System Command Block

        /// <summary>
        /// 1.1.x command block, processes system core operation messages
        /// </summary>
        /// <param name="state">Network state object</param>
        /// <param name="handle">User-defined command header</param>
        ...
        private void DataProcessingWithStartA( AppSession state, NetHandle handle, string data)
        {
            ...
            else if (handle == CommonHeadCode.SimplifyHeadCode.账户检查)
            {
                ...
                if (account.LoginEnable)
                {
                    if (!UserServer.ServerSettings.AllowUserMultiOnline)
                    {
                        ...
                        {
                            account.LoginEnable = false;
                            account.ForbidMessage = "This account is already logged in";
                        }
                    }
                }
                ...
                if(UserServer.ServerSettings.WhetherToEnableTrustedClientAuthentication)
                {
                    ...
                    {
                        account.LoginEnable = false;
                        account.ForbidMessage = "This client is not on the server's trusted list";
                    }
                }
            ...
            else if (handle == CommonHeadCode.SimplifyHeadCode.更新公告)
            {
                ...
                net_simplify_server.SendMessage(state, handle, "Success");
            }
            ...
            else if (handle == CommonHeadCode.SimplifyHeadCode.更新版本)
            {
                try
                {
                    ...
                    toolStripStatusLabel_version.Text = UserServer.ServerSettings.SystemVersion.ToString();
                    //记录数据
                    RuntimeLogHelper.WriteInfo($"version number has been changed：{data}");
            ...
            else if (handle == CommonHeadCode.SimplifyHeadCode.上传头像MD5)
            {
                ...
                {
                    net_simplify_server.SendMessage(state, handle, "Failure due to:" + ex.Message);
                }
            ...
            else if (handle == CommonHeadCode.SimplifyHeadCode.上传分厂)
            {
                try
                {
                    ...
                    RuntimeLogHelper?.WriteInfo("Uploaded branch information:" + data);
                }
                catch (Exception ex)
                {
        ...
        private void DataProcessingWithStartB( AppSession state, NetHandle handle, string data)
        {
            if (handle == CommonHeadCode.SimplifyHeadCode.网络日志查看)
            {
                ...
                RuntimeLogHelper.WriteInfo("View network logs");
            }
            ...
            else if (handle == CommonHeadCode.SimplifyHeadCode.文件日志清空)
            {
                ...
                net_simplify_server.SendMessage(state, handle, "Success");
                RuntimeLogHelper.WriteWarn("Clear shared file logs");
            }
            else if (handle == CommonHeadCode.SimplifyHeadCode.反馈日志查看)
            {
                LogNetSingle logNet = (LogNetSingle)AdviceLogHelper;
                net_simplify_server.SendMessage(state, handle, logNet.GetAllSavedLog());
                RuntimeLogHelper.WriteInfo("View suggestion and feedback logs");
            }
            else if (handle == CommonHeadCode.SimplifyHeadCode.反馈日志清空)
            {
                ...
                net_simplify_server.SendMessage(state, handle, "Success");
                RuntimeLogHelper.WriteWarn("Clear suggestion and feedback logs");
            }
            else if (handle == CommonHeadCode.SimplifyHeadCode.UDP日志查看)
            {
                ...
                RuntimeLogHelper.WriteInfo("View UDP logs");
            }
            ...
            else if (handle == CommonHeadCode.SimplifyHeadCode.头像日志清空)
            {
                ...
                net_simplify_server.SendMessage(state, handle, "Success");
                RuntimeLogHelper.WriteWarn("Clear avatar logs");
            }
            else
            {
                net_simplify_server.SendMessage(state, handle, data);
        ...
        /// <summary>
        /// Adds a new message, requiring the sender and message content to be specified.
        /// </summary>
        ...
        private void SoftMailInitialization()
        {
            ...
            SoftMail.MailSystem163.MailSendAddress = "linpy21@163.com";
        }
